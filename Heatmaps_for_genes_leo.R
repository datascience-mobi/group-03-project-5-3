...
g_pvalues_prov <- c()
for (i in 1:5) {
  g <- (kruskal.test(g_PC_batch_genes_leo[,i] ~ g_PC_batch_genes_leo[,6]))$p.value
...
for (j in 7:9) {
  g_pvalues <- c()
  for (i in 1:5) {
    g <- (permutationTest(g_PC_batch_genes_leo[,i], g_PC_batch_genes_leo[,j], nPermutation = 9999))$pVal.permut
...
pvalues_test1_gen_le <- data.frame(g_pvalues_prov, g_pvalues_date, g_pvalues_runs, g_pvalues_age, g_pvalues_Ct, g_pvalues_sex, row.names = c("PC1", "PC2", "PC3", "PC4", "PC5"))
colnames(pvalues_test1_gen_le) <- c("Provider", "Date", "Runs", "Age", "Cell type", "Sex")
pvalues_test_trans1_gen_le <- t(pvalues_test1_gen_le)
...
pvalues_test_norm1_gen_le <- apply(pvalues_test_trans1_gen_le
                           , c(1,2)
                           , norming_pvalues)
...
heatmap(as.matrix(pvalues_test_norm1_gen_le)
        , scale = "none"
        , col = heat.colors(2)
        , main = "K: P; Mon: D, R, A; Will: Ct, S; red < 0.01; [leo_genes], 1"
        , Rowv = NA
        , Colv = NA
)
...
heatmap(as.matrix(pvalues_test_trans1_gen_le)
        , scale = "none"
        , col = heat.colors(256)
        , main = "K: P; Mon: D, R, A; Will: Ct, S; red < 0.01; [leo_genes], 1"
        , Rowv = NA
...
for (j in 6:7) {
  ...
  for (i in 1:5) {
    g <- (kruskal.test(g_PC_batch_genes_leo[,i] ~ g_PC_batch_genes_leo[,j]))$p.value
...
pvalues_test2_gen_le <- data.frame(g_pvalues_prov, g_pvalues_date, g_pvalues_runs, g_pvalues_age, g_pvalues_Ct, g_pvalues_sex, row.names = c("PC1", "PC2", "PC3", "PC4", "PC5"))
colnames(pvalues_test2_gen_le) <- c("Provider", "Date", "Runs", "Age", "Cell type", "Sex")
pvalues_test_trans2_gen_le <- t(pvalues_test2_gen_le)
...
pvalues_test_norm2_gen_le <- apply(pvalues_test_trans2_gen_le
                           , c(1,2)
                           , norming_pvalues)
remove(norming_pvalues)
...
# Heatmap with two colors
heatmap(as.matrix(pvalues_test_norm2_gen_le)
        , scale = "none"
        , col = heat.colors(2)
        , main = "K: P, D; Mon: R, A; Will: Ct, S; red < 0.01; [leo_genes],2"
        , Rowv = NA
        , Colv = NA
)
...
# Heatmap with continous colors
heatmap(as.matrix(pvalues_test_trans2_gen_le)
        , scale = "none"
        , col = heat.colors(256)
        , main = "K: P, D; Mon: R, A; Will: Ct, S; red < 0.01; [leo_genes],2"
        , Rowv = NA
...
g_pvalues_age <- c()
for (i in 1:5) {
  g <- (kruskal.test(g_PC_batch_genes_leo[,i] ~ g_PC_batch_genes_leo[,9]))$p.value
...
pvalues_test3_gen_le <- data.frame(g_pvalues_prov, g_pvalues_date, g_pvalues_runs, g_pvalues_age, g_pvalues_Ct, g_pvalues_sex, row.names = c("PC1", "PC2", "PC3", "PC4", "PC5"))
colnames(pvalues_test3_gen_le) <- c("Provider", "Date", "Runs", "Age", "Cell type", "Sex")
pvalues_test_trans3_gen_le <- t(pvalues_test3_gen_le)
...
pvalues_test_norm3_gen_le <- apply(pvalues_test_trans3_gen_le
                           , c(1,2)
                           , norming_pvalues)
...
# Heatmap with two colors
heatmap(as.matrix(pvalues_test_norm3_gen_le)
        , scale = "none"
        , col = heat.colors(2)
        , main = "K: P, D, A; Mon: R; Will: Ct, S; red < 0.01; [leo_genes],3"
        , Rowv = NA
        , Colv = NA
)
remove(norming_pvalues)
...
heatmap(as.matrix(pvalues_test_trans3_gen_le)
        , scale = "none"
        , col = heat.colors(256)
        , main = "K: P, D, A; Mon: R; Will: Ct, S; red < 0.01; [leo_genes],3"
        , Rowv = NA
...
pvalues_test4_gen_le <- data.frame(g_pvalues_prov, g_pvalues_date, g_pvalues_runs, g_pvalues_age, g_pvalues_Ct, g_pvalues_sex, row.names = c("PC1", "PC2", "PC3", "PC4", "PC5"))
colnames(pvalues_test4_gen_le) <- c("Provider", "Date", "Runs", "Age", "Cell type", "Sex")
pvalues_test_trans4_gen_le <- t(pvalues_test4_gen_le)
...
pvalues_test_norm4_gen_le <- apply(pvalues_test_trans4_gen_le
                           , c(1,2)
                           , norming_pvalues)
...
# Heatmap with two colors
heatmap(as.matrix(pvalues_test_norm4_gen_le)
        , scale = "none"
        , col = heat.colors(2)
        , main = "K: P, D, R, A, Ct, S; red < 0.01; [leo_genes],4"
        , Rowv = NA
        , Colv = NA
)
remove(norming_pvalues)
...
heatmap(as.matrix(pvalues_test_trans4_gen_le)
        , scale = "none"
        , col = heat.colors(256)
        , main = "K: P, D, R, A, Ct, S; red < 0.01; [leo_genes],4"
        , Rowv = NA
